...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        // Listen for system wake event
        DistributedNotificationCenter.default().addObserver(self, selector: #selector(self.handleWakeup), name: NSNotification.Name("com.apple.powermanagement.wakedevice"), object: nil)
        
        _ = self.currentSpace()  // Create space-id window for current space
        ...
        self.statusItem.menu!.addItem(withTitle: "Memmon (Custom)", action: nil, keyEquivalent: "")
    ...
    // Handle wakeup event to restore windows
    @objc func handleWakeup() {
        // Save current state before attempting to restore (in case windows moved during sleep)
        self.saveState()
        self.restoreState()  // Call restoreState to reset windows when the computer wakes up
    }
    ...
    private func restoreState() {
        // Always restore the current active space, regardless of whether it's in spacesNeedRestore
        if let currentSpaceId = currentSpace() {
            // Add current space to visited spaces
            let currentSpaceWinNums = self.getWinIds()
            self.spacesVisited.formUnion(currentSpaceWinNums)
            
            // Always restore the active space
            for (pid, bounds) in self.state[self.numScreens] ?? [:] {
                self.setWindowSizes(pid, bounds.filter{ currentSpaceWinNums.contains($0.0) })
            }
            
            // Remove from spaces that need restore if it was in there
            self.spacesNeedRestore.remove(currentSpaceId)
        }
        
        // Also restore other spaces that need restoration
        for space in self.spacesNeedRestore {
            // We can only restore spaces we can access, which means the current space
            // Since we already restored the current space above, we can remove it from the need-restore set
    ...
    @objc func activeSpaceChanged(_ notification: Notification) {
        // Save state before trying to restore - this preserves window positions
        self.saveState()
        self.restoreState()
    }
